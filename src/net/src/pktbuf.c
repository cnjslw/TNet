...
static pktblk_t block_buffer[PKTBUF_BLK_CNT]; // 数据块空间,目前只支持固定数量的块分配
static mblock_t pktbuf_list; // 空闲包列表
static pktbuf_t pktbuf_buffer[PKTBUF_BUF_CNT]; // 数据包空间,通用只支持固定数量的数据包分配
...
{
    pktblk_t* first_block = (pktblk_t*)0; // 指向第一个块的指针
    pktblk_t* pre_block = (pktblk_t*)0; // 指向上一次插入链表的数据块在哪里,方便下一次插入到这个块之前
    ...
    while (size) {
        ...
        int curr_size = 0;
        if (add_front) {
            curr_size = size > PKTBUF_BLK_SIZE ? PKTBUF_BLK_SIZE : size; // 当需要的大小,是否大于块的最大可使用数据

            // 反向分配，从末端往前分配空间
            new_block->size = curr_size;
            new_block->data = new_block->payload + PKTBUF_BLK_SIZE - curr_size; // data指向当前分配空间的地址
            if (first_block) {
                // 将自己加在头部
            ...
            // 正向分配，从前端向末端分配空间
            new_block->size = curr_size; // 只分配需要的数据
            new_block->data = new_block->payload; // 块中的data指向数据的开头
            if (pre_block) {
                // 将自己添加到表尾
                nlist_node_set_next(&pre_block->node, &new_block->node); // 块尾插入数据包链表
            }
        }

        size -= curr_size; // 调整size
        pre_block = new_block; // 指向最近插入的块
    }
...
net_err_t pktbuf_init(void)
{
    ...
    // 将block_buffer开始的内存区域划分成PKTBUF_BLK_CNT个相同大小sizeof(pktblk_t)的内存块，然后用链表(block_list)链接起来
    // 同时建立基于block_list访问的同步与互斥锁
...
/**
 * @brief 前移位置. 如果跨越一个包，则仅移动到下一个包的开头
 */
