 ...
 * @return 校验和结果
 */
static uint16_t checksum_peso(const xipaddr_t *src_ip , const xipaddr_t * dest_ip , uint8_t protocol , uint16_t * buf , uint16_t len)
{
...
xnet_err_t xudp_bind(xudp_t* udp, uint16_t local_port) {
    ...
    for (curr = udp_socket, end = &udp_socket[XUDP_CFG_MAX_UDP]; curr < end; curr++) {
        if ((curr != udp) && (curr->local_port == local_port)) {//已经有UDP绑定该端口
